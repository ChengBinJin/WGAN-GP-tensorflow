...
def main(path_list, name_list):
    for idx_list, path in enumerate(path_list):
        wgan_8gaussian_paths = all_files_under(os.path.join('img', path[0]), extension='jpg', special='disc')
        wgan_25gaussian_paths = all_files_under(os.path.join('img', path[1]), extension='jpg', special='disc')
        wgan_swissroll_paths = all_files_under(os.path.join('img', path[2]), extension='jpg', special='disc')

        wgan_gp_8gaussian_paths = all_files_under(os.path.join('img', path[3]), extension='jpg', special='disc')
        wgan_gp_25gaussian_paths = all_files_under(os.path.join('img', path[4]), extension='jpg', special='disc')
        wgan_gp_swissroll_paths = all_files_under(os.path.join('img', path[5]), extension='jpg', special='disc')
        ...
        video_writer = cv2.VideoWriter(name_list[idx_list], fourcc, 10.0, (frame_shape[1]*3, frame_shape[0]*2))
        ...
        for idx in range(len(wgan_8gaussian_paths)):
            ...
            frame_1 = np.hstack([img_wgan8, img_wgan25, img_wgans])
            frame_2 = np.hstack([img_wgangp8, img_wgangp25, img_wgangps])
...
if __name__ == '__main__':
    path01 = ['wgan_8gaussians_True', 'wgan_25gaussians_True', 'wgan_swissroll_True',
              'wgan-gp_8gaussians_True', 'wgan-gp_25gaussians_True', 'wgan-gp_swissroll_True']
    path02 = ['wgan_8gaussians_False', 'wgan_25gaussians_False', 'wgan_swissroll_False',
